...
function check_mail(){
    if(isset($_POST['signupemail'])){
        ...
        $sql= "SELECT * FROM user_table WHERE email = '{$_POST['signupemail']}'";
        $result = $db->query($sql);
...
function sign_up(){
    if(isset($_POST['signupemail2']) && isset($_POST['signupfirstname']) && isset($_POST['signupsurname']) && isset($_POST['signupgender'])){
        $userimage="";
        if($_POST['signupgender'] == 'Male' || $_POST['signupgender'] == 'male' ){
            $userimage = "img/man.jpg";
            
        }else if($_POST['signupgender'] == 'Female' || $_POST['signupgender'] == 'female'){
            $userimage = "img/lady.jpg";
        }
        
        $signupcheck= "SELECT * FROM user_table WHERE email = '{$_POST['signupemail2']}' LIMIT 1";
        require_once('../scripts/connect.php');
        $result1 = $db->query($signupcheck);
        if($result1->num_rows != 1 ){
            $signup = "INSERT INTO user_table (first_name, last_name, email, gender, profile_pic, signup_date) VALUES ('{$_POST['signupfirstname']}','{$_POST['signupsurname']}','{$_POST['signupemail2']}','{$_POST['signupgender']}', '{$userimage}',NOW())";
             require_once('../scripts/connect.php');
			 $result2 = $db->query($signup);
		$dfltpassword = 'defaultPassword';
		
		$usermail = $_POST['signupemail2'];
...
/*   EMAIL NECESSARY 
    ...
    include_once 'vendor/autoload.php';     // should be require   
            
    $mail = new PHPMailer(true);
    $mail->Host = 'smtp.gmail.com';  // Specify servers
    $mail->SMTPAuth = true;                               // Enable SMTP authentication
    $mail->Username = 'techybanky@gmail.com';                 // SMTP username
    $mail->Password = 'Bankole1.';                           // SMTP password
    $mail->SMTPSecure = 'ssl';     
    $mail->Port = 465;
    $mail->setFrom('no-reply@kobowise.cf', 'Mailer');
    $mail->addAddress('kolesan2k@yahoo.com', 'Bankole');  
     
END OF FIRST PART OF EMAIL NECESSARY   */ 
            
 /* ADDITIONAL EMAIL STUFF NOT NECESSARY           
            
            //  $mail->addAddress('ngeopolis@gmail.com');               // Name is optional
...
//
//    
//Content   END OF ADDITIONAL STUFF    */
            
/*            EMAIL NECESSARY STUFF
...
//        $lastname = $_POST['signupsurname'] ;
//        $email = $_POST['signupemail2'];
//        $gender = $_POST['signupgender'];
//        
//            exit($firstname .' '. $lastname.' '.$email.' '.$gender.' From PHP');
//        }

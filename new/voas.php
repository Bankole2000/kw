...
$no_of_voas = "SELECT * FROM recieve_request_table WHERE amount_unmatched > 0";
$unmrecs = "SELECT SUM(amount_unmatched) AS amount_unmatched FROM recieve_request_table";
$offrecs = "SELECT SUM(amount_to_recieve) AS amount_to_recieve FROM recieve_request_table";
$givrecs = "SELECT SUM(amount_recieved) AS amount_recieved FROM recieve_request_table";
require_once('../scripts/connect.php');
        $result1 = $db->query($states);
        $result2 = $db->query($banks);
        $result3 = $db->query($no_of_voas);
		$result4 = $db->query($unmrecs);
        $result5 = $db->query($offrecs);
        $result6 = $db->query($givrecs);
		...
		$rec_count = $result3 ->num_rows;

		$res1row = $result4->fetch_array();
	$res2row = $result5->fetch_array();
	$res3row = $result6->fetch_array();
	$unmatched_recs = $res1row['amount_unmatched'];
	$offered_recs = $res2row['amount_to_recieve'];
	$given_recs = $res3row['amount_recieved'];
?>
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                ...
                <li class="nav-item">
                        <a href="voas.php" class="nav-link active">VoAs</a>
                    </li>
                <li class="nav-item">
   ...
   <section>
       <div style="padding-top:100px; padding-bottom:20px;">
           <div><div class="container"><div class="row center">
               <div class="col text-center" style="padding:10px;"><button class="btn btn-success" data-toggle="modal" data-target="#add_user">Add Users</button> &nbsp; Pending Eligibility Requests = <?php echo $rec_count;?> <br> 
               Total Amount Requested = <?php echo $given_recs;?><br>
               Total Amount Paid = <?php echo $offered_recs;?><br>
               Total Requests Unmatched = <?php echo $unmatched_recs;?><br></div>
               </div></div>
               <div class="container">
                   <div class="row">
...
<footer id="main-footer" class="bg-dark">
   ...
   <div class="modal fade text-dark" id="approve_request">
       <div class="modal-dialog">
           <div class="modal-content">
               <div class="modal-header">
                   <h5 class="modal-title">Approve VoA <span id="viewmodaltitle" class="text-primary"></span></h5>
                   
               </div>
               <div class="modal-body">
                       <div class="container">
                       <div class="row">
                       	<p><strong>Reciever Name: </strong><span id="rec_name"></span><br>
                       	<strong>Eligibility: </strong><span id="rec_amount"></span><br>
                       	<strong>Location: </strong><span id="rec_location1"></span><br>
                       	<strong>Gender: </strong><span id="rec_gender"></span><br>
                       	<strong>Strikes: </strong><span id="rec_strikes"></span><br>
                       	<strong>VoA: </strong><span id="rec_voa"></span><br>
							<strong>Date Requested: </strong><span id="rec_date"></span><br></p>
                       	<input type="hidden" id="rec_request_id">
                       </div>
                       <div class="alert alert-success fade show" style="display:none;" id="approve_success">
                          <strong>Success!</strong> Voa Successfully Approved 
                      </div>
                         <div class="alert alert-warning fade show" style="display:none;" id="unapprove_success">
                          <strong>Success!</strong> Voa UnApproved 
                      </div>
                         <div class="alert alert-warning fade show" style="display:none;" id="approve_fail">
                          <strong>Warning!</strong> database error 
                      </div>
                      <div class="alert alert-warning" style="display:none;" id="approve_error">
                           <strong>Oops!</strong> Something else wrong 
                      </div>
                   </div>
                      </div>
                   
                   <div class="modal-footer">
                       <button id="voa_approve" type='submit' class="btn btn-success" >Approve</button><button id="voa_unapprove" type='submit' class="btn btn-warning" >Unapprove</button>
                       <button id="close" type='submit' value="submit" class="btn btn-primary" data-dismiss="modal">Close</button>
                   </div>
           </div>
       </div>
   </div>
    ...
    if(firstname.length == ""){
     ...
     $('#incomplete').show('fast');
        ...
        $(document).on('click','.approve', function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    console.log(data);    
                    $('#rec_voa').html(data.voa_link);
                    $('#rec_amount').html(data.amount_requested);
                    $('#rec_name').html(data.firstname+' '+data.lastname);
                    $('#rec_date').html(data.date_requested);
                    $('#rec_gender').html(data.gender);
                    $('#rec_strikes').html(data.user_strikes);
                    $('#rec_location1').html(data.state);
                    $('#rec_request_id').val(data.recieve_request_id);
        ...
        $('#voa_approve').click(function(){
			var action = 'approve_voa';
    	var recid = $('#rec_request_id').val();
            ...
            $.ajax({
              ...
              action:action,
                ...
                console.log(response);
                    if(response == 'successful'){
                         $('#approve_success').show('fast');
            
              window.setTimeout(function () { 
                            $("#approve_success").hide('fast'); location.reload();}, 2000);  
                                              
                        }else if(response == 'failed'){
                      $('#approve_fail').show('fast');
                  window.setTimeout(function () { 
                            $("#approve_fail").hide('fast'); }, 2000); 
                        }else{
							$('#approve_error').show('fast');
                  window.setTimeout(function () { 
                            $("#approve_error").hide('fast'); }, 2000); 
						}} 
                    });
                });
        
		
        $('#voa_unapprove').click(function(){
			var action = 'unapprove_voa';
    	var recid = $('#rec_request_id').val();
            ...
            $.ajax({
              ...
              action:action,
                ...
                console.log(response);
                    if(response == 'successful'){
                         $('#unapprove_success').show('fast');
            
              window.setTimeout(function () { 
                            $("#unapprove_success").hide('fast'); location.reload();}, 2000);  
...
//    update button click      
        $('#rec_approve_btn').click(function(){
        var recid = '' ; 
        var action = 'approve_voa';

...
$get_users = "SELECT * FROM user_table JOIN banks ON user_table.bank_id = banks.bank_id JOIN states ON user_table.state_id = states.state_id ORDER BY first_name ASC";
        $resultd = $db->query($get_users);
        $users = '';
        while($rowd = $resultd->fetch_array())
        {               
            $users .= '<option userid="'.$rowd["user_id"].'">'.$rowd["user_id"].' - '.$rowd["first_name"].' - '.$rowd["email"].' - &#8358;'.$rowd["eligibility"].' - '.$rowd["state"].' - strikes - '.$rowd["user_strikes"].'</option>';
        };
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                ...
                <li class="nav-item">
                         <a href="matchm.php" class="nav-link active">Match Me</a>
                    </li>
                       <li class="nav-item">
  ...
  <section style="padding-top:100px; padding-bottom:20px;">
    ...
    <select class="form-control" id="user_select">
      <?php echo $users;?>
                 ...
                 <label for="match_amount">Amount &nbsp; <span id="lmatch_amount"></span></label>
    ...
    <input type="number" pattern="[0-9]*" class="form-control" id="match_amount" placeholder="100,000 Maximum">
              ...
              <div class="container">
               ...
               <button class="btn btn-primary btn-block" id="make_match">GIVE ELIGIBILITY</button>
          </div>
      </div></div>
  </section>
   ...
   <section>
       <div style="padding-top:0px; padding-bottom:20px;">
           <div><div class="container"><div class="row center">
               <div class="col text-center" style="padding:10px;"><button class="btn btn-success" data-toggle="modal" data-target="#add_user">Add Users</button></div>
               </div></div>
               <div class="container">
                   <div class="row">
...
// Give Eligibility
        ...
        $("#make_match").click(function(){
            var don_reg = /^[0-9]{4,}$/i;
            var user_id = $('option:selected',"#user_select").attr('userid');
            var match_amount = $("#match_amount").val();
            if(match_amount == 0 || match_amount == ''){
                $("#lmatch_amount").addClass('text-danger').html('Empty').removeClass('text-success');
				$("#match_amount").removeClass('valid').addClass('invalid');
                $('#match_empty').show('fast');
              window.setTimeout(function () { 
                            $("#match_empty").hide('fast'); }, 2000);
                console.log(match_amount);
               };
           
             
            if(don_reg.test(match_amount)){
           console.log(match_amount);
                var action = "admin_give_eligibility";
                ...
                $("#lmatch_amount").removeClass('text-danger').html('Eligibility Given').addClass('text-success');
				$("#match_amount").removeClass('invalid').addClass('valid');
